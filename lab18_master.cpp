...
{
    int size, rank1, rank2, slaves_size;
    MPI_Status status;
    MPI_Comm intercomm;
    char slave[15] = "./lab18_slave";
    MPI_Init(&argc, &argv);
    ...
    MPI_Recv(&slaves_size, 1, MPI_INT, 2, 2, intercomm, &status);
    printf("Slave #%d: slaves_size=%d\n", status.MPI_SOURCE, slaves_size);
    MPI_Finalize();
    return 0;
}